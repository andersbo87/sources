 ...
 * Author: andersbo
 *
 * Created on May 21, 2015, 1:48 PM
 */
...
void printInfo(int curr, int all, int remaining){
    ...
    if(status.compare("partition") == 0){
        fprintf(stdout, "Partition: %d of %d items (%g%% complete)\n", curr, all, percent);
    }
    else if(status.compare("quicksort") == 0){
        fprintf(stdout, "QuickSorting %d of %d.\n", curr, all);
    }
    else
        fprintf(stdout, "Driver på med en eller annen operasjon.\n");
}
...
{
  prog_name = "qs";
  randomizeQuicksort *qs = new randomizeQuicksort();
  if(argc != 2){
  ...
  fprintf(stdout, "\n");
  qs->randomizedQuicksort(arr,0,l-1);
