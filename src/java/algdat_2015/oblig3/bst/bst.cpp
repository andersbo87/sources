...
int main(int argc, char **argv){
    ...
    try
    {
        ...
        if(argc == 3)
        {
	      ...
	      tree->slettNode(atoi(del_str[i].c_str()));
            ...
            if(mtime_del == 0){
                ...
                fprintf(stdout, "Programmet brukte %ld mikrosekunder på å slette %d noder.",mtime_del_usec, del_tot);
            }
            else
        ...
        fprintf(stderr, "%s: Noe gikk galt: %s\n", prog_name, e.what());
        void *arr[100];
        size_t s = backtrace(arr, sizeof(arr) / sizeof(arr[0]));
