        ...
        for(i = 1; i <= instot; i++)
	{
	  ...
	  signal(SIGINFO, catch_siginfo);
        ...
        if(argc == 3)
        {
            ...
            for(i=1; i<= deltot; i++)
            {
	      //fprintf(stdout, "Skal til å fjerne node med verdi %d\n", atoi(del_str[i].c_str()));
	      thisNode = i;
	      allNodes = deltot;
	      status = "sletting";
	      #ifdef SIGINFO
